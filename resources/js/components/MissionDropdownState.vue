...
export default {
  ...
  methods: {
    ...
    onSubmitState(state) {
      ...
      if (state == 'Terminée') {
        this.message = `Les participations en attente de validation seront automatiquement déclinées et celles validées passeront au statut mission effectuée.<br><br>Les volontaires seront notifiés de ces modifications.<br><br> Êtes vous sûr de vouloir continuer ?`
      }

      if (state == 'Annulée') {
        this.message = `Attention, vous êtes sur le point d'annuler une mission en lien avec ${this.form.participations_count} participation(s).<br><br> Les participations liées seront automatiquement annulées et les volontaires inscrits seront notifiés de l'annulation de la mission.<br><br> Êtes vous sûr de vouloir continuer ?`
      }
