...
function removeIndex(routeState) {
  if (`${process.env.algolia.index}` in routeState)
    routeState = routeState[`${process.env.algolia.index}`]
...
function addIndex(routeState) {
  routeState = {
    [`${process.env.algolia.index}`]: routeState,
  }
  return routeState
}
