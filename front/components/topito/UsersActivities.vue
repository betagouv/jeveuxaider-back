<template>
  <CardTopito
    title="Utilisateurs les plus actifs"
    subtitle="Nombre d'actions sur la plateforme"
    :loading="loading"
  >
    <template slot="actions"
      ><el-select
        v-model="filters.role"
        placeholder="Tous les rôles"
        clearable
        class="w-28"
        size="small"
        @change="onChangeOption"
      >
        <el-option label="Référent départemental" value="referent_department" />
        <el-option label="Référent régional" value="referent_region" />
        <el-option label="Admin" value="admin" />
      </el-select>
    </template>
    <ul v-if="items.length" class="divide-y divide-gray-200">
      <router-link
        v-for="(item, index) in items"
        :key="item.id"
        class="p-4 flex justify-between items-center hover:bg-gray-50 cursor-pointer"
        :to="`/dashboard/profile/${item.profile.id}`"
        target="_blank"
      >
        <div class="flex">
          <Avatar
            :source="item.profile.image ? item.profile.image.thumb : null"
            :fallback="item.profile.short_name"
          />
          <div class="ml-3">
            <div class="text-sm font-medium text-gray-900">
              {{ item.profile.full_name }}
            </div>
            <div class="text-sm text-gray-500">
              {{ item.count | formatNumber }}
              {{ item.count | pluralize(['action', 'actions']) }}
            </div>
          </div>
        </div>
        <div class="text-gray-400 font-bold text-sm">
          {{ index + 1 }}
        </div>
      </router-link>
    </ul>
    <div v-else class="flex items-center justify-center p-6">
      <div class="text-gray-500">Pas de résultats</div>
    </div>
  </CardTopito>
</template>

<script>
import CardTopito from '@/components/CardTopito'

export default {
  components: {
    CardTopito,
  },
  props: {
    daterange: {
      type: String,
      required: true,
    },
  },
  data() {
    return {
      loading: true,
      items: [],
      filters: { daterange: this.daterange, role: null },
    }
  },
  watch: {
    daterange(newValue, oldValue) {
      this.filters.daterange = newValue
      this.fetch()
    },
  },
  created() {
    this.fetch()
  },
  methods: {
    onChangeOption(value) {
      this.filters.role = value
      this.fetch()
    },
    async fetch() {
      this.loading = true
      const response = await this.$api.fetchTopitoUtilisateursLesPlusActifs(
        this.filters
      )
      this.items = response.data.items
      this.loading = false
    },
  },
}
</script>

<style></style>
