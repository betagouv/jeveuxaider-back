<template>
  <div class="bg-gray-100">
    <Breadcrumb :items="[{ label: 'Territoires engagés', link: '/territoires' }]" />
    ...
    <div class="py-8 md:py-12 lg:py-20 bg-gray-50 overflow-hidden shadow-lg">
      <div class="relative max-w-xl mx-auto lg:max-w-screen-xl">
        <div class="px-4">
          <img class="hidden lg:block absolute transform translate-y-1 opacity-50" style="left: 100%; transform: translateX(-75%) !important" src="/images/france.svg" width="904" alt="">

          <div class="relative">
            <h1 class="text-center text-3xl md:text-5xl lg:text-6xl leading-none font-bold tracking-tight text-gray-900">
            ...
            <p class="mt-4 text-center max-w-2xl text-xl leading-7 text-gray-500 lg:mx-auto">
              Sur l'ensemble du territoire français, des milliers de réservistes, de structures et d'association ont déjà rejoint
              ...
              <form class="w-full flex md:ml-0 mb-0" action="#" method="GET">
                ...
                <div class="relative w-full text-cool-gray-400 focus-within:text-cool-gray-600">
                  <div class="absolute inset-y-0 left-0 flex items-center pointer-events-none">
                    <svg class="h-5 w-5 ml-3 sm:ml-6" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="relative my-8 sm:px-4">
          <nav class="flex overflow-scroll sm:overflow-visible pt-1 pb-3 sm:py-0 sm:justify-center sm:gap-4">
            <span
              ...
              :class="[
                {
                  'text-white bg-blue-800 focus:text-white focus:bg-blue-800': activeType == type.slug,
                },
                {
                  'text-gray-500 hover:text-blue-800 bg-white': activeType != type.slug,
                },
                {
                  'mr-4 sm:mr-0': types[index].slug == types[types.length - 1].slug,
                },
              ]"
              class="
                px-3
...
export default {
  ...
  data () {
    ...
    return {
      ...
      groups: {
        ...
        departments: [['A'], ['B', 'C'], ['D', 'E', 'F'], ['G', 'H'], ['I'], ['J', 'L'], ['M', 'N'], ['O', 'P'], ['R', 'S', 'T'], ['V', 'Y']]
      }
    }
  },
  head () {
    return {
      title: 'Inscrivez votre territoire sur JeVeuxAider.gouv.fr, la plateforme publique du bénévolat proposée par la Réserve Civique',
      link: [
        {
          rel: 'canonical',
  ...
  methods: {
    typeCount (type) {
      return this[type].filter(item => this.slugify(item.name).includes(this.slugify(this.query))).length
    },
    territoriesByGroup (group) {
      return this[this.activeType].filter(territory => group.includes(territory.name[0]) && this.slugify(territory.name).includes(this.slugify(this.query))).sort((a, b) => (a.name < b.name ? -1 : 1))
    },
    slugify (str) {
      const map = {
        '-': ' ',
