...
class MissionPolicy
{
    ...
    {
        if (Mission::role('responsable')->where('id', $mission->id)->count() > 0 && request()->header('Context-Role') === 'responsable' && !empty($user->profile->mobile) && $user->profile->doesntHave('missionsOffline')) {
            return true;
        }
