...
class MissionController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
        ...
        if (is_numeric($id)) {
            $mission = Mission::with(['structure.members:id,first_name,last_name,mobile,email', 'template', 'template.domaine', 'domaine', 'domaineSecondary', 'responsable', 'skills', 'template.photo', 'illustrations', 'structure.illustrations', 'structure.overrideImage1'])->withCount('temoignages')->where('id', $id)->first();
            if ($mission) {
