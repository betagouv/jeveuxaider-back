...
class StructureInDraft extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        ...
        if ($this->template == 'j+7') {
            return (new MailMessage)
            ->subject($notifiable->profile->first_name.', finalisez votre inscription pour recruter des bénévoles')
            ->greeting('Bonjour,')
        ...
        if ($this->template == 'j+15') {
            return (new MailMessage)
            ->subject($notifiable->profile->first_name.', des bénévoles vous attendent')
            ->greeting('Bonjour,')
