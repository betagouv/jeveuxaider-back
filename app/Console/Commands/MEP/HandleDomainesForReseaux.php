...
class HandleDomainesForReseaux extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'mep:handle-domaines-for-reseaux';
    ...
    {
        $this->info('Ce script va créer les liaisons des reseaux vers les domaines (domainables)');

        if ($this->confirm('Continuer ?')) {
            $this->handleReseauDomains();
        }
    }

    private function handleReseauDomains()
    {
        $query = Taggable::where('taggable_type', "App\Models\Reseau")
            ->where('tag_id', '<=', 11);
